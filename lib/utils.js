...
export function generateCommentary(matchData) {
  ...
  if (
    matchData.participants[0].penaltyShoot[0] ||
    matchData.participants[1].penaltyShoot[0]
  ) {
    ...
    if (matchData.status === "PLAYED") {
      ...
      if (
        matchData.participants[0].penaltyShoot.length >
        matchData.participants[1].penaltyShoot.length
      ) {
        winner = team1;
      } else if (
        matchData.participants[0].penaltyShoot.length <
      ...
      commentaryArray.push(`The winner of the match is ${winner}`);
    }
  }
